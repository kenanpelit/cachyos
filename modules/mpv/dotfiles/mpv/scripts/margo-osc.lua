...
local user_opts = {
    ...
    indicator_icon = "pause",              -- "pause" or "play"
    indicator_stay = true,                 -- keep indicator visible during pause
    indicator_timeout = 0.6,               -- timeout (s) if indicator doesn't stay
    keybind_allow = false,                 -- default off: glyph visual only, modernz owns mbtn_left
    keybind_set = "mbtn_left",             -- keybind to toggle pause (when keybind_allow)
    keybind_mode = "onpause",              -- "onpause" or "always"
    keybind_eof_disable = true,            -- disable keybind on eof
    ...
    rectangles_width = 30,                 -- pause icon: bar width
    rectangles_height = 80,                -- pause icon: bar height
    rectangles_spacing = 20,               -- pause icon: spacing between bars
    triangle_width = 80,                   -- play icon: width
    triangle_height = 80,                  -- play icon: height
    flash_play_icon = true,                -- flash play icon on unpause
    flash_icon_timeout = 0.3,              -- timeout (s) for flash icon
    ...
    mute_indicator_pos = "middle_right",   -- mute indicator position
...
-- ══════════════════════════════════════════════════════════════════════════
-- Merkez pause göstergesi (pause_indicator_lite.lua'dan entegre)
-- Kendi ortamında (do...end) çalışır; kendi overlay'leri + observer'ları.
-- Ayarlar margo-osc.conf'tan okunur. keybind_allow varsayılan=false:
-- glyph sadece görsel, mbtn_left tıklamasını modernz yönetir.
-- ══════════════════════════════════════════════════════════════════════════
do
-- Pause göstergesi ayarları modernz'in user_opts'una taşındı (tek config
-- namespace → "unknown key" uyarısı yok). Aşağıdaki 'options' onu alias'lar.
local options = user_opts
