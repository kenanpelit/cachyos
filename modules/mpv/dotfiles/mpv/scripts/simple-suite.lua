-- simple-suite.lua
--
-- SimpleHistory + SimpleBookmark + SmartCopyPaste_II'yi TEK dosyada birleştirir.
-- Üçü aynı liste motorunu paylaşır; her biri kendi ortamında (setfenv) çalışır,
-- böylece state'leri (o, list_contents, ...) ve bağlama adları birbirinden bağımsız.
--
-- Yapı: motor ve üç gövde, aşağıda `local function _engine/_history/...` içine
-- sarmalanmıştır. spawn() her örnek için taze bir ortam kurar, motoru ve gövdeyi
-- o ortamda çalıştırır. Sarmalayıcı-fonksiyon içinde tanımlanan global fonksiyonlar
-- (bind_keys, draw_list, ...) setfenv sayesinde o ortama düşer.
--
-- Ayarlar: TEK script-opts/simple-suite.conf. Motorun read_list_options'ı,
-- örnek önekiyle (history_/bookmark_/clipboard_) o dosyayı kendisi ayrıştırır.
...
-- simple-suite/bookmark.lua — SimpleBookmark gövdesi
-- Kendi ortamında çalışır; liste motoru engine.lua'dan gelir.
-- scripts/simple-suite.lua tarafından yüklenir.
...
log_length_text = 'length='
log_time_text = 'time='
log_keybind_text = 'slot='
log_group_text = 'group='
protocols = {'https?:', 'magnet:', 'rtmps?:', 'smb:', 'ftps?:', 'sftp:'}
available_sorts = {'added-asc', 'added-desc', 'time-asc', 'time-desc', 'alphanum-asc', 'alphanum-desc'}
search_string = ''
search_active = false
loadTriggered = false --1.3.0# to identify if load is triggered atleast once for idle option
resume_selected = false
list_contents = {}
list_start = 0
list_cursor = 1
...
function parse_header(str)
	...
	str = str:gsub("%%total%%", #list_contents)
		:gsub("%%cursor%%", list_cursor)
...
function search_log_contents(arr_contents)
	...
	for i = 1, #arr_contents do --1.3# removed specific search method as it doesn't seem useful anymore, --1.3.1# utilize arr_contents instead of list_contents
...
function get_list_contents(filter, sort)
	...
	if filter_log_contents(list_contents, filter) then list_contents = filter_log_contents(list_contents, filter) end
	if search_log_contents(list_contents) then list_contents = search_log_contents(list_contents) end
...
function draw_list(arr_contents)
	...
	if search_active and not arr_contents[1] then --1.3.1# changed list_contents to arr_contents
		osd_msg = osd_msg .. 'No search results found' .. osd_msg_end
	end
...
function load(list_cursor, add_playlist, target_time)
	if not list_contents or not list_contents[1] then return end
	if not target_time then
		if not list_contents[#list_contents - list_cursor + 1] then return end --1.3.0# fixes crash when loading an entry that doesn't exist
		seekTime = tonumber(list_contents[#list_contents - list_cursor + 1].found_time) + o.resume_offset 
		if (seekTime < 0) then
			seekTime = 0
		end
	...
	if file_exists(list_contents[#list_contents - list_cursor + 1].found_path) or starts_protocol(protocols, list_contents[#list_contents - list_cursor + 1].found_path) then
		...
		if o.osd_messages == true then
			mp.osd_message('File Doesn\'t Exist:\n' .. list_contents[#list_contents - list_cursor + 1].found_path) --1.3# cant use the list_filepath because file doesn't exist so it returns nil
		end
		msg.info('The file below doesn\'t seem to exist:\n' .. list_contents[#list_contents - list_cursor + 1].found_path)
		return
	end
...
function delete_selected()
	...
	local list_sequence = list_contents[#list_contents - list_cursor + 1].found_sequence
	local list_filepath = list_contents[#list_contents - list_cursor + 1].found_path or ""
	local list_seektime = tonumber(list_contents[#list_contents - list_cursor + 1].found_time) or 0
...
function remove_additional_param_log_entry(index, target, log_text)
	...
	local log_index = list_contents[target].found_sequence
...
function list_slot_remove(index, action)
	if not list_drawn then return end
	if not list_contents or not list_contents[1] then return end
	if not index then index = tonumber(list_contents[#list_contents - list_cursor + 1].found_slot) end
...
function list_slot_add(index)
	...
	local cursor_filename, cursor_filepath, cursor_filetitle = get_local_names(list_contents[#list_contents - list_cursor + 1]) --1.2.4# added the new name calling method to fix issue of unable to add urls into groups or slots
	...
	local slotIndex = list_contents[#list_contents - list_cursor + 1].found_slot
	if slotIndex then
		remove_additional_param_log_entry(slotIndex,#list_contents-list_cursor+1, log_keybind_text)
	end
	
	list_slot_remove(index, 'silent')
	add_additional_param_log_entry(index, #list_contents-list_cursor+1, log_keybind_text)
...
function list_group_remove(action)
	...
	remove_additional_param_log_entry(groupCursorIndex, #list_contents-list_cursor+1, log_group_text)
		...
		for j=1, #list_highlight_cursor do
			if list_contents[#list_contents+1-i] then
				if list_contents[#list_contents+1-i].found_sequence == list_highlight_cursor[j][2].found_sequence then
					...
					if groupIndex then
						remove_additional_param_log_entry(groupIndex, #list_contents+1-i, log_group_text)
...
function list_group_add(index)
	...
	add_additional_param_log_entry(index, #list_contents-list_cursor+1, log_group_text)
...
function list_group_add_highlighted(index)
	...
	for i = 1, #list_contents do
		for j=1, #list_highlight_cursor do
			if list_contents[#list_contents+1-i] then
				if list_contents[#list_contents+1-i].found_sequence == list_highlight_cursor[j][2].found_sequence then
					add_additional_param_log_entry(index, #list_contents+1-i, log_group_text)
...
function list_group_add_cycle(action)
	...
	local next_index = tonumber(list_contents[#list_contents - list_cursor + 1].found_group)
...
function add_load_slot(key_index)
	...
	if list_drawn then
		...
		if list_contents ~= nil and list_contents[1] then
			for i = 1, #list_contents do
				if tonumber(list_contents[i].found_slot) == key_index then
				...
				if o.keybinds_empty_auto_create then
					if filePath ~= nil then
						if o.keybinds_empty_fileonly then
							...
							get_list_contents() --1.2.9# reflect removal of key_index in write_log function, fixes bug and cleaner code -- also used get_osd instead of local because add_additional_param uses osd_log inside its function perhaps this should be changed
							local current_slot = tonumber(list_contents[#list_contents].found_slot) --1.2.9# gets the slot of the current item
							...
							add_additional_param_log_entry(key_index, #list_contents, log_keybind_text) --1.2.9# adds the slot of the passed index
						else
							write_log(false) --1.2.9# reflect removal of key_index in write_log function, fixes bug and cleaner code
							get_list_contents() --1.2.9# reflect removal of key_index in write_log function, fixes bug and cleaner code
							local current_slot = tonumber(list_contents[#list_contents].found_slot) --1.2.9# gets the slot of the current item
							remove_all_additional_param_log_entry(current_slot, log_keybind_text) --1.2.9# removes all the slots of the current item
							remove_all_additional_param_log_entry(key_index, log_keybind_text) --1.2.9# removes all the slots that are going to be added based on passed index
							add_additional_param_log_entry(key_index, #list_contents, log_keybind_text) --1.2.9# adds the slot of the passed index
...
mp.observe_property("idle-active", "bool", function(_, v)
	...
	if v and type(o.load_item_on_startup) == "number" and not loadTriggered then --1.3.0# option to immediately load an entry based on number
		...
		list_contents = read_log_table() --1.3.0# get the item list to use load function
		if not list_contents or not list_contents[1] then return end

		if o.load_item_on_startup == -1 then o.load_item_on_startup = #list_contents end --1.3.0# specify -1 as last entry
...
-- ── örnekleri ayağa kaldır ──────────────────────────────────────────────────
local function spawn(engine_fn, body_fn, conf_prefix, prefix)
    ...
    setfenv(engine_fn, env)
    engine_fn()

    setfenv(body_fn, env)
    local ok, err = pcall(body_fn)
    if not ok then
        msg.error('simple-suite (' .. prefix .. ') calistirilamadi: ' .. tostring(err))
    end
end

spawn(_engine, _history,   'history_',   'history-')
spawn(_engine, _bookmark,  'bookmark_',  'bookmark-')
spawn(_engine, _clipboard, 'clipboard_', 'clipboard-')
