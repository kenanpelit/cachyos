...
echo "Initializing telegram on $WM_TYPE..."
...
APP_PID=$!
mkdir -p "$STATE_DIR"
echo "$APP_PID" > "$STATE_DIR/telegram.pid"
echo "$COMMAND" > "$STATE_DIR/telegram.cmd"
...
if [[ "$WORKSPACE" != "0" && "$WM_TYPE" == "hyprland" ]] && command -v hyprctl >/dev/null 2>&1 && command -v jq >/dev/null 2>&1; then
    ...
    ELAPSED=0
    CLASS_PATTERN="Telegram|telegram-desktop|org.telegram.desktop"
...
echo "telegram initialization complete"
exit 0
