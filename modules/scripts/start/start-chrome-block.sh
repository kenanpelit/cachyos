...
readonly ARGS_STR="block --separate --restore-last-session"
...
echo "Initializing chrome-block on $WM_TYPE..."
...
APP_PID=$!
mkdir -p "$STATE_DIR"
echo "$APP_PID" > "$STATE_DIR/chrome-block.pid"
echo "$COMMAND" > "$STATE_DIR/chrome-block.cmd"
...
echo "chrome-block initialization complete"
exit 0