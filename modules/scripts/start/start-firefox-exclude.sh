...
readonly ARGS_STR="-P exclude --class exclude --name exclude --new-window --new-instance"
...
echo "Initializing firefox-exclude on $WM_TYPE..."
...
APP_PID=$!
mkdir -p "$STATE_DIR"
echo "$APP_PID" > "$STATE_DIR/firefox-exclude.pid"
echo "$COMMAND" > "$STATE_DIR/firefox-exclude.cmd"
...
echo "firefox-exclude initialization complete"
exit 0
