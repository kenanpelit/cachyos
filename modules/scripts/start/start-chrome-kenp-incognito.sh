...
readonly ARGS_STR="kenp --separate --restore-last-session --incognito"
...
echo "Initializing chrome-kenp-incognito on $WM_TYPE..."
...
APP_PID=$!
mkdir -p "$STATE_DIR"
echo "$APP_PID" > "$STATE_DIR/chrome-kenp-incognito.pid"
echo "$COMMAND" > "$STATE_DIR/chrome-kenp-incognito.cmd"
...
echo "chrome-kenp-incognito initialization complete"
exit 0