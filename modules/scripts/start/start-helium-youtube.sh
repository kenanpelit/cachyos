...
readonly ARGS_STR="--youtube --separate --class helium-youtube.com__-Default"
...
echo "Initializing helium-youtube on $WM_TYPE..."
...
APP_PID=$!
mkdir -p "$STATE_DIR"
echo "$APP_PID" > "$STATE_DIR/helium-youtube.pid"
echo "$COMMAND" > "$STATE_DIR/helium-youtube.cmd"
...
if [[ "$WORKSPACE" != "0" && "$WM_TYPE" == "hyprland" ]] && command -v hyprctl >/dev/null 2>&1 && command -v jq >/dev/null 2>&1; then
    ...
    ELAPSED=0
    CLASS_PATTERN="helium-youtube.com__-Default"
...
echo "helium-youtube initialization complete"
exit 0