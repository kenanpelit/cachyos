...
readonly ARGS_STR="--class rmpc -T rmpc -e rmpc"
...
echo "Initializing rmpc on $WM_TYPE..."
...
APP_PID=$!
mkdir -p "$STATE_DIR"
echo "$APP_PID" > "$STATE_DIR/rmpc.pid"
echo "$COMMAND" > "$STATE_DIR/rmpc.cmd"
...
echo "rmpc initialization complete"
exit 0