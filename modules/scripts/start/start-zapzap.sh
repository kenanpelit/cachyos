#!/usr/bin/env bash
# Profile: zapzap
# Managed start script (Semsumo-compatible)
set -e
...
readonly COMMAND="env"
readonly ARGS_STR="QT_QPA_PLATFORM=wayland XDG_SESSION_TYPE=wayland zapzap --wayland"
...
echo "Initializing zapzap on $WM_TYPE..."
...
APP_PID=$!
mkdir -p "$STATE_DIR"
echo "$APP_PID" > "$STATE_DIR/zapzap.pid"
echo "$COMMAND" > "$STATE_DIR/zapzap.cmd"
...
if [[ "$WORKSPACE" != "0" && "$WM_TYPE" == "hyprland" ]] && command -v hyprctl >/dev/null 2>&1 && command -v jq >/dev/null 2>&1; then
    ...
    ELAPSED=0
    CLASS_PATTERN="zapzap|ZapZap|com.rtosta.zapzap"
...
echo "zapzap initialization complete"
exit 0
