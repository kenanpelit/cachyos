          ...
          Row {
            ...
            Row {
              ...
              Item {
                ...
                //
                // PATCH (cachy local override): the dots widget originally
                // bound visibility / length to `passwordInput.text.length`,
                // which is *per-WlSessionLockSurface*. On multi-monitor
                // setups the lock surface that holds wl_keyboard focus is
                // the one under the cursor, so its passwordInput fills as
                // the user types — but the OTHER monitor's passwordInput
                // only gets the text via the LockContext sync Connection,
                // and Qt's QML binding through that sync isn't reliable
                // for invisible TextInput items. The result: PAM auth
                // works (lockContext.currentText is correct) but the dots
                // don't render. `lockControl` IS the shared LockContext
                // instance, so its `currentText` is the canonical source
                // of truth across both surfaces.
                Item {
                  width: Math.min(passwordDisplayContent.width, 550)
                  height: 20
                  visible: lockControl.currentText.length > 0 && !passwordInputContainer.passwordVisible
                  anchors.left: parent.left
                  anchors.verticalCenter: parent.verticalCenter
                  clip: true
                  ...
                  Row {
                    ...
                    Repeater {
                      id: iconRepeater
                      // PATCH (cachy override): drive the dot count from
                      // the shared lockControl state, not the per-surface
                      // passwordInput, so all monitors render the same
                      // number of dots regardless of which surface is
                      // currently holding wl_keyboard focus.
                      model: ScriptModel {
                        values: Array(lockControl.currentText.length)
                      }
