...
- Runtime assets live under `~/.config/mango/runtime/` and are regenerated by
  `modules/mangowm/scripts/ensure-runtime-files.sh`.
...
- `generated/theme.conf` is repo-owned and derived from `theme/theme.env`.
- `runtime/profile.conf`, `runtime/workspace-rules.conf`, and
  `runtime/workspace-binds.conf` are user-runtime assets and should be
  regenerated, not edited.
- `runtime/keybind-cheatsheet.conf` is a generated compatibility layer for the
  Noctalia keybind overlay while Mango is active.
